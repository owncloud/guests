...
OCA.Guests.App = {
	...
	initSharingGuests: function($el) {
		...
		this._guestsFileList.appName = t('files_sharing', 'Shared with Guests');
		this._guestsFileList.$el.find('#emptycontent').html('<div class="icon-share"></div>' +
			'<h2>' + t('files_sharing', 'Nothing shared with Guests yet') + '</h2>' +
			'<p>' + t('files_sharing', 'Files and folders you share with Guests will show up here') + '</p>');
		return this._guestsFileList;
	},
